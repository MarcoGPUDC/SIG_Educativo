...
if($tipo != null and $tipo != "todo"){
	...
	if($nivel != null and $nivel != "todo"){
		...
		if(str_contains(strtolower($nivel), 'secundari')) {
   			 $join = "JOIN ra2020.plan_estudio_local p ON j.id_oferta_local = p.id_oferta_local JOIN ra2020.plan_estudio_local_secundaria n ON p.id_plan_estudio_local = n.id_plan_estudio_local JOIN codigos.orientacion_tipo o ON n.c_orientacion = o.c_orientacion JOIN codigos.dictado_tipo t ON p.c_dictado = t.c_dictado";
		...
		if(str_contains(strtolower($nivel), 'snu')) {
   			 $join = "JOIN (SELECT DISTINCT pl.id_oferta_local, u.descripcion , dt.descripcion as Dictado FROM ra2020.oferta_local j JOIN ra2020.plan_estudio_local pl ON j.id_oferta_local = pl.id_oferta_local JOIN ra2020.plan_estudio_local_superior pls ON pls.id_plan_estudio_local = pl.id_plan_estudio_local JOIN ra2020.titulo_oferta s ON pl.c_titulo_oferta = s.c_titulo_oferta JOIN ra2020.titulo t ON t.id_titulo = s.id_titulo JOIN codigos.carrera_tipo u ON t.c_carrera = u.c_carrera  JOIN codigos.dictado_tipo dt ON pl.c_dictado = dt.c_dictado WHERE j.c_oferta = '115') as ofe ON ofe.id_oferta_local = j.id_oferta_local";
...
$query = "SELECT DISTINCT Ori.Dictado as dictado
