...
async function getGeoserverLayer(workspace, layer) {
	const viewLayer = L.tileLayer.wms("http://sistemas2.chubut.edu.ar/geoserver/ows", {
	...
	let dataLayer;
	try {
		const geoResponse = await fetch(`http://sistemas2.chubut.edu.ar/geoserver/sigeducativo/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=${workspace}%3A${layer}&maxFeatures=300&outputFormat=application%2Fjson&srsname=EPSG:4326`);
